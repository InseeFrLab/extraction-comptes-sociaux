...
def objects_to_structures(objects, tokens, class_thresholds):
    ...
    for table in tables:
        ...
        table_tokens = [
            token
            for token in tokens
            if iob(token["bbox"], table["bbox"]) >= 0.5
        ]

        structure = {}

        columns = [
            obj for obj in table_objects if obj["label"] == "table column"
        ]
    ...
    for spanning_cell in spanning_cells:
        ...
        for subcell in subcells:
            ...
            if (
                ...
                header = (
                    header
                    and "column header" in subcell
                    and subcell["column header"]
                )
...
def cells_to_html(cells):
    ...
    return str(
        ET.tostring(table, encoding="unicode", short_empty_elements=False)
    )
...
def objects_to_cells(
    ...
    if (
        len(table_structures["columns"]) < 1
...
def objects_to_table_structures(
    ...
    columns = refine_columns(
        columns, tokens_in_table, class_thresholds["table column"]
    )
...
def nms_by_containment(
    container_objects, package_objects, overlap_threshold=0.5
):
    """
...
def extract_text_inside_bbox(spans, bbox):
    ...
    bbox_text = extract_text_from_spans(
        bbox_spans, remove_integer_superscripts=True
    )
...
def refine_table_structures(
    table_bbox, table_structures, page_spans, class_thresholds
):
    """
...
def table_structure_to_cells(table_structures, table_spans, table_bbox):
    ...
    for supercell in supercells:
        ...
        for subcell in subcells:
            ...
            subcell_rect_area = subcell_rect.get_area()
            if (
                subcell_rect.intersect(supercell_rect).get_area()
                / subcell_rect_area
            ) > 0.5:
                if cell_rect is None:
                    cell_rect = Rect(list(subcell["bbox"]))
